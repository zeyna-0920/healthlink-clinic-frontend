    ...
    <div className="bg-[image:var(--gradient-soft)]">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-16">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between mb-10">
          <div>
            <p className="text-sm uppercase tracking-[0.3em] text-medical font-semibold mb-3">Notifications</p>
            <h1 className="text-4xl font-bold">Notifications</h1>
            <p className="mt-3 text-muted-foreground max-w-2xl">
              Consulter les rappels, confirmations et alertes envoyées aux patients.
            </p>
          </div>
          <Badge variant={unreadCount > 0 ? "destructive" : "success"}>
            {unreadCount} non lus
          </Badge>
        </div>

        {loading ? (
          <Card className="p-10 text-center text-muted-foreground">Chargement des notifications…</Card>
        ) : error ? (
          <Card className="p-10 text-center text-destructive">{error}</Card>
        ) : notifications.length === 0 ? (
          <Card className="p-10 text-center text-muted-foreground">Aucune notification pour le moment.</Card>
        ) : (
              ...
              <Card key={notification._id} className="p-6">
                <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h2 className="text-lg font-semibold">{notification.title || notification.type}</h2>
