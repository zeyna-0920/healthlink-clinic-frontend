...
function BedsPage() {
  ...
  const [statistics, setStatistics] = useState({ total: 0, occupied: 0, available: 0, maintenance: 0 });
    ...
    <div className="bg-[image:var(--gradient-soft)]">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-16">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between mb-10">
          <div>
            <p className="text-sm uppercase tracking-[0.3em] text-medical font-semibold mb-3">Gestion des lits</p>
            <h1 className="text-4xl font-bold">Lits</h1>
            <p className="mt-3 text-muted-foreground max-w-2xl">
              Visualisez l'état des lits et suivez immédiatement la disponibilité dans chaque service.
            </p>
          </div>
            ...
            {beds.map((bed) => (
              <Card key={bed._id} className="p-6">
                <div className="flex items-start justify-between gap-4">
                  ...
                  <Badge variant={bed.isOccupied ? "destructive" : bed.status === "maintenance" ? "warning" : "success"}>
                    {bed.status}
                  </Badge>
                </div>
                ...
                {bed.notes ? <p className="mt-4 text-sm text-muted-foreground">Note : {bed.notes}</p> : null}
              </Card>
            ))}
          </div>
        )}
